    ...
    COMP: Component,
{
    ...
    for ev in Parser::new_ext(
        ...
        match ev {
                            ...
                            for (i, c) in vtag.childs.iter_mut().enumerate() {
                                if let &mut VNode::VTag(ref mut vtag) = c {
                                    match aligns[i] {
                                        Alignment::None => {}
                                        Alignment::Left => {
                                            vtag.add_class("text-left")
                                        }
                                        Alignment::Center => {
                                            vtag.add_class("text-center")
                                        }
                                        Alignment::Right => {
                                            vtag.add_class("text-right")
                                        }
                                    }
                                }
                            }
            ...
            Event::Text(text) => {
                add_child!(VText::new(text.to_string()).into())
            }
